...
def dual_test_env(agent: torch.nn.Module, test_envs: gym.vector.SyncVectorEnv, cog_test_envs: gym.vector.SyncVectorEnv,n_runs, device):
    agent.eval()
    ...
    while count_episode < n_runs:
        ...
        obs, reward, done, info = test_envs.step(action.cpu().numpy()[:, 8])
        cog_obs, cog_reward, cog_done, cog_info = cog_test_envs.step(action.cpu().numpy()[:,-1])
