...
def main(max_steps=3000, velocity_per_step=10, plot_type='all', seed=10, model_path="./hrl_bs_ijcnn2023/models/SmallLowGearAntTRP-v0__ppo__0__1741201287.pth", render=False):
...
if __name__ == "__main__":
	...
	parser.add_argument('--render', action='store_true', help='Render the environment during the test')
	...
	if args.multi_seed:
		...
		for seed in seed_values:
			...
			torch.manual_seed(seed)
			np.random.seed(seed)	
			main(max_steps=args.max_steps, velocity_per_step=args.velocity_per_step, plot_type=args.plot_type, seed=seed, model_path=args.model_path, render=args.render)
	else:
		print("Running with single seed for testing...")
		main(max_steps=args.max_steps, velocity_per_step=args.velocity_per_step, plot_type=args.plot_type, seed=10, model_path=args.model_path, render=args.render)